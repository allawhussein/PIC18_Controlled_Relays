...
void Screen_Date_Time()
{
    ...
    RTC->sec = ((RTC->sec & 0x0F) + (((RTC->sec>>4) & (0x0F)) * 10));
    RTC->min = ((RTC->min & 0x0F) + (((RTC->min>>4) & (0x0F)) * 10));
    RTC->hour = ((RTC->hour & 0x0F) + (((RTC->hour>>4) & (0x0F)) * 10));




    if(RTC->sec < 10) sprintf(secs, "0%d ", RTC->sec);
    else sprintf(secs, "%d ", RTC->sec);
